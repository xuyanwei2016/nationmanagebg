      ...
      <el-table :data="list" :key='tablekey' v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="handleSelectionChange" @current-change="handleCurChange" ref="multipleTable">
            ...
            <el-table-column label="类型" align="center" width="200">
              <template slot-scope="scope">
                <span v-if="scope.row.score===0">收入</span>
                <span v-else style="color: red;">支出</span>
              </template>  
            </el-table-column>
            <el-table-column label="交易金额（元）" align="center" width="150">
                <template slot-scope="scope">{{ scope.row.money}}</template>
            </el-table-column>
            <el-table-column label="余额（元）" align="center" width="200">
...
<script>
  ...
  import { searchBalanceAPI } from "@/api/member/personMember";
  ...
  export default{
    ...
    mounted(){
      this.listQuery.memberId = this.$route.query.account
    ...
    methods:{
      getList(listQuery){
        ...
        searchBalanceAPI(this.listQuery).then(response => {
