  ...
  <div class="app-container calendar-list-container">
    ...
    <el-form inline class="demo-form-inline">
      <el-form-item label="友情链接标题">
        <el-input @keyup.enter.native="handleFilter" style="width: 400px;" @clear="clear" placeholder="链接标题" class="filter-item" v-model="listQuery.title" clearable>
        </el-input>
      </el-form-item>
      <el-form-item>
      ...
      if(this.multipleSelection.length>0){
        confirm.apply(this,['确定要删除选择的友情链接吗?']).then(() => {
          deleteFriendlink(this.massQuery.ids).then(res => {
            operationMsg.apply(this,[res.data.code,`批量删除${this.multipleSelection.length}项友情链接`])
          })
        })
      } else {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          addFriendlink(this.temp).then(res => {
            operationMsg.apply(this,[res.data.code,'友情链接创建'])
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateFriendlink(tempData).then(res => {
            operationMsg.apply(this,[res.data.code,'友情链接更新'])
            this.dialogFormVisible = false
          });
        }
      });
