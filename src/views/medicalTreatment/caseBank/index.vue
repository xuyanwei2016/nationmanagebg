  ...
  <el-container>
    <el-main>
      ...
      <el-table :key='tableKey' :data="list" v-loading="listLoading" @sort-change="handleSort" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="chioceList" >
        ...
        <el-table-column label="作者" prop="author" min-width="180" align="center" ></el-table-column>
        <el-table-column label="添加时间" min-width="180" align="center" prop="createTime"></el-table-column>
        ...
        <el-table-column class-name="small-padding fixed-width" label="操作" width="200" align="center">
          <template slot-scope="scope">
            ...
            <i :class="authBtns.filter(i => i.idName=='deleteBtn')[0].icon" :title="authBtns.filter(i => i.idName=='deleteBtn')[0].name" v-if="authBtns.filter(i => i.idName=='deleteBtn').length > 0 && scope.row.status == 0" @click="deleteBtn(scope.row.id)"></i>
          </template>
        </el-table-column>
      </el-table>
